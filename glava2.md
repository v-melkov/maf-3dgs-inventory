...
**Таблица 4. Обобщённая схема пайплайна сбора и обработки данных**
...
#### 2.5.3. Программная реализация

Этап реализован средствами модуля Geo Register Plugin для LichtFeld Studio, распространяемого на условиях лицензии GNU GPL v3. Модуль реализует описанное преобразование подобия по схеме RANSAC + IRLS и поддерживает несколько источников исходных геоданных: автоматическое извлечение координат из EXIF-тегов изображений, загрузку предварительно вычисленного преобразования из файла JSON, загрузку координат из файла CSV, а также импорт уточнённых положений камер из результатов уравнивания в программных продуктах RealityScan и Agisoft Metashape.

В настоящем исследовании применяется режим автоматического извлечения координат из EXIF-тегов, что соответствует принятой схеме подготовки данных (раздел 2.4.4). Сопоставление изображений с восстановленными позами камер выполняется по имени файла без расширения.

Параметры устойчивого решателя задаются в конфигурационном файле модуля. Существенными для настоящей задачи являются пороговое значение невязки для отбраковки соответствий и параметр функции потерь Хьюбера; с учётом использования приёмника потребительского класса указанные параметры принимаются исходя из ожидаемой точности исходных координат.

Результатом работы модуля является файл преобразования, содержащий значения коэффициента масштаба, матрицы поворота, вектора переноса, а также характеристики качества решения — среднеквадратическую невязку и число соответствий, признанных достоверными. Указанные характеристики фиксируются как показатели качества геопривязки и включаются в состав атрибутивных данных объекта (раздел 2.6).

#### 2.5.4. Обоснование раздельного определения масштаба
...
#### 2.5.5. Определение масштабного коэффициента по линейному эталону
...
1. На кадрах, содержащих эталон, опознаются характерные точки шкалы — деления с известным расстоянием между ними.
2. По восстановленным на этапе SfM параметрам камер выполняется пространственная засечка опознанных точек, в результате чего определяются их координаты в системе координат модели.
3. Вычисляется расстояние d<sub>м</sub> между точками в единицах модели.
4. Масштабный коэффициент определяется как k = d<sub>э</sub> / d<sub>м</sub>, где d<sub>э</sub> — известное расстояние между теми же делениями эталона.
...
#### 2.5.6. Экспорт геопривязанной модели

Модуль геопривязки обеспечивает экспорт модели в форматы, пригодные для использования в геоинформационных системах:
...
Соотношение полей по источнику формирования составляет 12 автоматически формируемых полей против 8 заполняемых оператором, при этом автоматически формируются все поля, требующие измерений, тогда как участие оператора ограничивается классификационными и организационными сведениями. Данное соотношение характеризует степень автоматизации разработанной методики и используется при оценке трудоёмкости (раздел 3.4).